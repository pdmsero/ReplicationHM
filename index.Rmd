...
$$\begin{equation} Y_{i}=A_{i}L_{i}^{\alpha}K_{i}^{\eta}T_{i}^{1-\alpha-\eta}, \end{equation}$$which can be used to write the "profit function" for each city:	  $$\begin{equation}\Pi_{i}=Y_{i}-W_{i}L_{i}-R_{i}K_{i}-P_{i}T_{i}=0.\end{equation}$$It is important to set this profit to zero because "cities" are assumed to be competitive and not earn excess rents.
First-order conditions for this problem yield:  $$\begin{align*}\alpha Y_{i}&=W_{i}L_{i}\\ \eta Y_{i}&=R_{i}K_{i}\\ (1-\alpha-\eta) Y_{i}&=P_{i}T_{i}\end{align*}$$We can use these first order conditions to write:
$$\alpha R_{i}K_{i}=\eta W_{i}L_{i}$$
and from here replace $K_{i}$ in the conditions above: $$\begin{align*}\alpha Y_{i}=\alpha A_{i}L_{i}^{\alpha}K_{i}^{\eta}T_{i}^{1-\alpha-\eta}=W_{i}L_{i}\Rightarrow \\ \Rightarrow \alpha A_{i}L_{i}^{\alpha}\left(\frac{\eta}{\alpha}\frac{W_{i}}{R_{i}}L_{i}\right)^{\eta}T_{i}^{1-\alpha-\eta}=W_{i}L_{i}\Rightarrow \\ \Rightarrow L_{i}^{1-\alpha-\eta}=A_{i}\left(\frac{\alpha}{W_{i}}\right)^{1-\eta}\left( \frac{\eta}{R_{i}}\right)^{\eta}T_{i}^{1-\alpha-\eta}\Rightarrow \\ \Rightarrow L^{d}_{i}=A_{i}^{\frac{1}{1-\alpha-\eta}}T_{i}\left[\left(\frac{\alpha}{W_{i}}\right)^{1-\eta}\left( \frac{\eta}{R_{i}}\right)^{\eta} \right]^{\frac{1}{1-\alpha-\eta}}\end{align*}$$
## Perfect mobility
### $\underline{\text{Labour supply}}$
Indirect utility is assumed to be given by:
$$V=\frac{W_{i}Z_{i}}{P_{i}^{\beta}},$$with house prices driven by the following equation:$$P_{i}=\bar{P}_{i}L_{i}^{\gamma_{i}},$$where $\gamma_{i}$ is the elasticity of housing supply with respect to the total labour supply, and $\bar{P}_{i}$ the component of the house price that does not depend on utility.
...
$$\begin{equation} Y_{i}=A_{i}L_{i}^{\alpha}K_{i}^{\eta}T_{i}^{1-\alpha-\eta}. \end{equation}$$The first order condition for labour implies:
$$\begin{equation} \alpha Y_{i}=W_{i}L_{i}, \end{equation}$$
which can be aggregated over all $i$ to result in:
$$\begin{equation} \sum_{i}\alpha Y_{i}=\sum_{i}W_{i}L_{i} \end{equation} \Rightarrow \alpha = \frac{\sum_{i}W_{i}L_{i}}{\sum_{i} Y_{i}}$$From the definition of output, we can replace $Y_{i}$ with the FOC with respect to labour and the level of capital using the optimal relationship between capital and labour:
